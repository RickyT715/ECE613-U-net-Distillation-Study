...
def teacher_validate_metrics(model, dataloader, device):
    ...
    evaluator = MetricCollection({
        ...
        "Specificity": Specificity(num_classes=2, task="multiclass"),
        "Recall": Recall(num_classes=2, task="multiclass"),
        "JaccardIndex": JaccardIndex(num_classes=2, task="multiclass"),
    }).to(device)
...
def main():
    ...
    #初始化学生
    student = StudentUCTransUNet(in_channels=3, out_channels=2).to(device)
