...
from train_distillation.train_distillation_unet import train_distillation, best_result
import torchmetrics
...
def main():
    config = load_config("../configs/distillation/unet.yaml")
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    # Teacher
    teacher = UNet(**config["model"]["params"]).to(device)
    teacher.load_state_dict(torch.load(config["distillation"]["teacher_model_path"], map_location=device))
    teacher.eval()

    # Student
    student = StudentUNet(**config["model"]["params"]).to(device)
