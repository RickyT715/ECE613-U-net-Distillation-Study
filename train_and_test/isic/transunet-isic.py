...
CONFIG_NAME = "isic/isic2018_transunet.yaml"
...
from models.transunet.vit_seg_modeling import VisionTransformer as Net
...
model = Net(config_vit, **config['model']['params']).cuda()
...
if "R50-ViT-B_16".find('R50') != -1:
    config_vit.patches.grid = (int(INPUT_SIZE / 16), int(INPUT_SIZE / 16))
best_model = Net(config_vit, **config['model']['params']).cuda()
best_model.load_from(weights=np.load(config_vit.pretrained_path))
